...
defined('ABSPATH') || exit; ?>
    ...
    <table class="shop_table woocommerce-checkout-review-order-table">
        ...
        <tr>
            ...
            <th class="product-total"><?php esc_html_e('Preis', 'woocommerce'); ?></th>
        </tr>
        </thead>
        <tbody>
        <?php
                    ...
                    <td class="product-name">
                        ...
                        if (!empty($cart_item['articles_reserved_by_admin'])) {
                            ...
                            foreach ($cart_item['articles_reserved_by_admin'] as $item) {
                                $days = $item['days'];
                                $booking_days = BOOKINGPORT_StandStatusHandler::get_booking_day_name($days);
                                $product_name = "{$item['street']} {$item['number']} ({$booking_days}) <br/>";
                                $counter = 0;

                                if (!empty($item['width'])) {
                                    $product_name .= 'Breite: ' . $item['width'] . 'm' . '<br/>';
                                }
                                if (!empty($item['depth'])) {
                                    $product_name .= 'Tiefe: ' . $item['depth'] . 'm';
                                }

                            }
                        } else {
                            $product_name = $cart_item['street'] . ' ' . $cart_item['number']  . ' (' . $booking_days . ')';
                        }
